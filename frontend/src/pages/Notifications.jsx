    ...
    <div className="p-6 md:p-8 bg-gradient-to-br from-blue-50 via-green-50 to-purple-50 rounded-2xl">
      ...
      {canSend && me?.role !== 'super_admin' && (
        <div className="flex items-center gap-2 mb-6">
          <button onClick={()=>setActiveTab('incoming')} className={`px-4 py-2 rounded-xl border transition ${activeTab==='incoming'?'bg-gradient-to-r from-blue-600 to-green-600 text-white shadow-md':'bg-white/70 hover:bg-white border-gray-200'} `}>Incoming</button>
          <button onClick={()=>setActiveTab('outgoing')} className={`px-4 py-2 rounded-xl border transition ${activeTab==='outgoing'?'bg-gradient-to-r from-blue-600 to-green-600 text-white shadow-md':'bg-white/70 hover:bg-white border-gray-200'} `}>Outgoing</button>
        </div>
      )}

      {/* Staff (Doctor/Office Executive) Composer (Outgoing tab) */}
      {activeTab === 'outgoing' && (me?.role === 'doctor' || me?.role === 'office_executive') && (
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-md border border-white/60 p-6 mb-6">
      ...
      {activeTab === 'outgoing' && drafts.length > 0 && (
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-md border border-white/60 p-4 mb-6">
      ...
      {/* Filters row */}
      <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-md border border-white/60 p-4 mb-4 flex flex-col md:flex-row gap-3 items-stretch">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
          <input value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} placeholder="Search title or message..." className="w-full pl-10 pr-4 py-2.5 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-300" />
        </div>
        <div className="flex gap-2">
          <select value={filterType} onChange={(e)=>setFilterType(e.target.value)} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300">
          ...
          <select value={filterStatus} onChange={(e)=>setFilterStatus(e.target.value)} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300">
            <option value="all">All</option>
            <option value="unread">Unread</option>
            <option value="read">Read</option>
          </select>
          <select value={filterClinic} onChange={(e)=>setFilterClinic(e.target.value)} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300">
            <option value="all">All Clinics</option>
            {clinics.map(c => <option key={c.id || c._id} value={c.id || c._id}>{c.name}</option>)}
          </select>
        </div>
        <div className="flex gap-2">
          <input type="date" value={dateFrom} onChange={(e)=>setDateFrom(e.target.value)} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300" />
          <input type="date" value={dateTo} onChange={(e)=>setDateTo(e.target.value)} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300" />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          [...Array(5)].map((_, i) => (
            <div key={i} className="bg-gradient-to-r from-gray-100 to-gray-50 rounded-2xl p-6 animate-pulse h-24"></div>
          ))
          ...
          filteredNotifications.map((n) => (
            <div
              ...
              className={`p-6 rounded-2xl transition-all duration-300 flex items-start gap-4 ${
                n.is_read ? "bg-gradient-to-r from-white to-gray-50 shadow-sm" : "bg-gradient-to-r from-blue-50 to-green-50 border border-blue-200 shadow"
              }`}
            >
              {!n.is_read && (
